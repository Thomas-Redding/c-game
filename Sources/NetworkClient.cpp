...
#include "../Headers/NetworkClient.hpp"

NetworkClient::NetworkClient(ServerCommunicator& com, ClientServerCommunicator &comB, NetworkClock *net_clock) : communicator(com), offlineCommunicator(comB) {
	tcpSocket.setBlocking(false);
	udpSocket.setBlocking(false);
	udpSocket.bind(sf::Socket::AnyPort);
	network_clock = net_clock;
}
...
void NetworkClient::networkUpdate() {
	...
	while(true) {
		...
		if(message != "") {
			...
			if(message.at(0) == '_') {
				...
				tcpMessageReceived(message, network_clock->serverToClientTime(timeStamp));
		}
		else
			break;
	}
	...
	while(true) {
		...
		if(message != "") {
			...
			if(message.at(0) == '_') {
				long currentTime = network_clock->getLocalTime();
				...
				*/
				network_clock->add(timeStamp - (timeSent + currentTime)/2);
			}
			else
				udpMessageReceived(message, network_clock->serverToClientTime(timeStamp));
		}
		else {
			break;
		}
	...
	if(connectionState >= 3) {
		...
		if(currentTime - timeLastPingSent > 200 || (currentTime - timeLastPingSent > 30 && network_clock->size() < 10)) {
			std::string message = std::to_string(network_clock->getLocalTime()) + "@_PING___";
...
void NetworkClient::sendTcpMessage(std::string message) {
	if(connectionState >= 1) {
		if(message == "Quit Server") {
			...
			message = std::to_string(network_clock->getServerTime()) + "@" + message;
