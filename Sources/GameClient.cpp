...
#include "../Headers/GameClient.hpp"

GameClient::GameClient(int *currentPageNumber, sf::RenderWindow *w, sf::Font *myFont, NetworkClock *net_clock) : Page(currentPageNumber, w, myFont), view(w), systemsHandler(false) {
...
bool GameClient::update() {
	...
	long deltaTime = network_clock->getLocalTime() - timeOfLastFrame;
	timeOfLastFrame = network_clock->getLocalTime();
	...
	long diff = network_clock->estimateClockDiff();
...
void GameClient::tcpMessageReceived(std::string message, long timeStamp) {
	...
	if(vect[0] == "startGame") {
		...
		timeOfLastFrame = network_clock->getServerTime();
		currentInputState.timeStamp = timeOfLastFrame;
...
void GameClient::udpMessageReceived(std::string message, long timeStamp) {
	...
	newEntities.timeStamp = network_clock->serverToClientTime(newEntities.timeStamp);
	...
	long t = network_clock->getLocalTime();
