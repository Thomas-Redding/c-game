...
Client::Client(sf::RenderWindow &myWindow, ServerCommunicator &com, ClientServerCommunicator &comB, NetworkClock *net_clock) : window(myWindow), NetworkClient(com, comB, net_clock) {
...
bool Client::start() {
	...
	pages.push_back(new GameClient(&currentPage, &window, &font, network_clock));
